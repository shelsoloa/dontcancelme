    ...
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex items-center gap-2">
        <input
          ...
          className="w-24 rounded-lg border border-line-strong bg-transparent px-2 py-1.5 text-sm outline-none focus:border-primary"
        />
      ...
      <button
        ...
        className="inline-flex h-9 items-center justify-center rounded-full bg-primary px-4 text-sm font-medium text-primary-ink transition-opacity hover:opacity-90 disabled:opacity-50"
      >
        {loading ? "Loading…" : "+ Top Up"}
      </button>
      {belowMin && (
        <p className="mt-1 w-full text-sm text-crit">
          Minimum 500 credits / transaction
        </p>
      )}
      {!belowMin && error && (
        <p className="mt-1 w-full text-sm text-crit">{error}</p>
      )}
    </div>
  );
}
